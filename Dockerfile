...
RUN ( grep -E -i "^${NAGIOS_GROUP}"    /etc/group || groupadd $NAGIOS_GROUP    ) && \
    ( grep -E -i "^${NAGIOS_CMDGROUP}" /etc/group || groupadd $NAGIOS_CMDGROUP )
...
RUN wget --progress=dot:giga -O ${NAGIOS_HOME}/libexec/check_ncpa.py https://raw.githubusercontent.com/NagiosEnterprises/ncpa/${NCPA_BRANCH}/client/check_ncpa.py && \
...
RUN cd /opt                                                                                  && \
    pip install --no-cache-dir --break-system-packages pymssql paho-mqtt                                    && \
...
RUN cd /tmp                                                                                                                                                   && \
    wget --progress=dot:giga https://github.com/chriscareycode/nagiostv-react/releases/download/v${NAGIOSTV_VERSION}/nagiostv-${NAGIOSTV_VERSION}.tar.gz && \
